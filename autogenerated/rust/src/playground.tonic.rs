...
pub mod spotify_client {
    ...
    impl SpotifyClient<tonic::transport::Channel> {
        ...
        <T::ResponseBody as Body>::Error: Into<StdError> + Send,
    {
        ...
        pub fn with_interceptor<F>(
            inner: T,
            interceptor: F,
        ) -> SpotifyClient<InterceptedService<T, F>>
        where
        ...
        pub async fn stream_recently_played_song(
            ...
            tonic::Response<tonic::codec::Streaming<super::GetRecentlyPlayedSongResponse>>,
            tonic::Status,
        > {
            self.inner.ready().await.map_err(|e| {
                tonic::Status::new(
...
pub mod spotify_server {
    #![allow(unused_variables, dead_code, missing_docs, clippy::let_unit_value)]
    use tonic::codegen::*;
    /// Generated trait containing gRPC methods that should be implemented for use with SpotifyServer.
    #[async_trait]
    pub trait Spotify: Send + Sync + 'static {
        ...
        type StreamRecentlyPlayedSongStream: tonic::codegen::tokio_stream::Stream<
                Item = std::result::Result<super::GetRecentlyPlayedSongResponse, tonic::Status>,
            > + Send
            + 'static;
    ...
    impl<T: Spotify> SpotifyServer<T> {
        pub fn new(inner: T) -> Self {
    ...
    impl<T, B> tonic::codegen::Service<http::Request<B>> for SpotifyServer<T>
    where
        T: Spotify,
        B: Body + Send + 'static,
        ...
        fn call(&mut self, req: http::Request<B>) -> Self::Future {
            match req.uri().path() {
                "/playground.Spotify/GetRecentlyPlayedSong" => {
                    ...
                    let fut = async move {
                        let method = GetRecentlyPlayedSongSvc(inner);
                        let codec = tonic::codec::ProstCodec::default();
                ...
                "/playground.Spotify/StreamRecentlyPlayedSong" => {
                    ...
                    impl<T: Spotify>
                        tonic::server::ServerStreamingService<super::GetRecentlyPlayedSongRequest>
                        for StreamRecentlyPlayedSongSvc<T>
                    {
                        ...
                        fn call(
                            ...
                            let inner = Arc::clone(&self.0);
                            let fut = async move {
                                <T as Spotify>::stream_recently_played_song(&inner, request).await
                            };
                            Box::pin(fut)
                        }
                    }
    ...
    impl<T: Spotify> Clone for SpotifyServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    impl<T: Spotify> tonic::server::NamedService for SpotifyServer<T> {
        const NAME: &'static str = "playground.Spotify";
    }
}
