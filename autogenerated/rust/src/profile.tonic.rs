...
pub mod profile_client {
    ...
    impl ProfileClient<tonic::transport::Channel> {
        ...
        <T::ResponseBody as Body>::Error: Into<StdError> + Send,
    {
        ...
        pub fn with_interceptor<F>(
            inner: T,
            interceptor: F,
        ) -> ProfileClient<InterceptedService<T, F>>
        where
...
pub mod profile_server {
    #![allow(unused_variables, dead_code, missing_docs, clippy::let_unit_value)]
    use tonic::codegen::*;
    /// Generated trait containing gRPC methods that should be implemented for use with ProfileServer.
    #[async_trait]
    ...
    impl<T: Profile> ProfileServer<T> {
        pub fn new(inner: T) -> Self {
    ...
    impl<T, B> tonic::codegen::Service<http::Request<B>> for ProfileServer<T>
    where
        T: Profile,
        B: Body + Send + 'static,
    ...
    impl<T: Profile> Clone for ProfileServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    impl<T: Profile> tonic::server::NamedService for ProfileServer<T> {
        const NAME: &'static str = "profile.Profile";
    }
}
