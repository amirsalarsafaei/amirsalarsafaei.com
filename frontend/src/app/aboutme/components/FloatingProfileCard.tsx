...
export default function FloatingProfileCard({
  ...
  return (
    ...
    >
      ...
      >
        <meshStandardMaterial color="#43D9AD" metalness={0.2} roughness={0.8} />
      </RoundedBox>
      ...
      <Text
        ...
        font="/fonts/JetBrainsMono-Regular.ttf"
      >
        Learning/Building distributed systems and high performance applications
      </Text>
      ...
      {Array.from({ length: 8 }).map((_, i) => (
        <FloatingParticle key={i} index={i} hovered={hovered} />
      ))}

      {Array.from({ length: 10 }).map((_, i) => (
        <DecorativeShape key={`shape-${i}`} index={i} hovered={hovered} />
      ))}
    </group>
  );
}
  ...
  useFrame((state) => {
    if (particleRef.current) {
      ...
      particleRef.current.position.x =
        baseX + Math.cos(time * speed) * amplitude;
      particleRef.current.position.y =
        Math.sin(time * 0.7 + index) * (hovered ? 0.6 : 0.3);
  ...
  useFrame((state) => {
    if (shapeRef.current) {
      ...
      shapeRef.current.position.x =
        basePosition[0] + Math.sin(time * speed) * amplitude;
      shapeRef.current.position.y =
        basePosition[1] + Math.cos(time * speed * 0.7) * amplitude;
      shapeRef.current.position.z =
        basePosition[2] + Math.sin(time * speed * 0.5) * amplitude;
