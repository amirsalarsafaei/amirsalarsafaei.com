...
export default function BlogEditor({
  ...
  const { data: availableTags = [], refetch: refetchTags } = useQuery({
    queryKey: ["tags"],
    queryFn: async () => {
      const response = await tags_client.ListTags(
        ListTagsRequest.create(),
        new grpc.Metadata({ ...(token && { authorization: token }) }),
      );
  ...
  const createTagMutation = useMutation({
    mutationFn: async (tagName: string) => {
      ...
      const response = await tags_client.CreateTag(
        request,
        new grpc.Metadata({ ...(token && { authorization: token }) }),
      );
      return response;
    ...
    <div className={styles.upsertContainer}>
      <div className={styles.upsertHeader}>
        ...
        <div className={styles.tagsSection}>
          <div className={styles.tagsHeader}>
            <h3>Tags</h3>
            <Button onClick={handleCreateTag} variant="terminal" size="sm">
              Add Tag
            </Button>
          </div>
      ...
      <div style={{ flexGrow: 1 }}>
        <div className={styles.editorContainer}>
          ...
          <div
            className={`${styles.editorPane} ${isPreviewMode ? styles.hidden : ""}`}
          >
            <textarea
              ref={textareaRef}
              value={markdown}
          ...
          <div
            className={`${styles.previewPane} ${isPreviewMode ? styles.visible : ""}`}
          >
