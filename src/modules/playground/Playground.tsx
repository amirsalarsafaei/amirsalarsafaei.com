...
import {GLTFResult} from './models/Gopher.tsx'
import {Model as Linux} from './models/Linux.tsx';
  ...
  useFrame((state) => {
    if (modelRef.current) {
      ...
      if (leftEye && rightEye && leftPupil && rightPupil) {
        ...
        // Calculate direction to camera for each eye
        ...
        // Look at camera
        const lookAtLeft = cameraPosition.clone().sub(leftEyePosition);
        const lookAtRight = cameraPosition.clone().sub(rightEyePosition);

        // Apply rotation limits

        // Eye radius (distance from eye center to pupil)
        const maxOffset = 0.15; // Maximum pupil movement
        ...
        const leftOffsetX = Math.max(-maxOffset, Math.min(maxOffset, -lookAtLeft.x * 0.1));
        const leftOffsetY = Math.max(-maxOffset, Math.min(maxOffset, -lookAtLeft.y * 0.1));
        ...
        const rightOffsetX = Math.max(-maxOffset, Math.min(maxOffset, -lookAtRight.x * 0.1));
        const rightOffsetY = Math.max(-maxOffset, Math.min(maxOffset, -lookAtRight.y * 0.1));
